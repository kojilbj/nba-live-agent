...
def _games_for_today_raw() -> dict:
    ...
    """
    try:
        sb = _with_retry(lambda: scoreboard.ScoreBoard())
...
def _games_for_date_raw(date: str) -> dict:
    ...
    try:
        ...
        sb = _with_retry(lambda: scoreboardv2.ScoreboardV2(game_date=date))
...
def _game_status(game_id: str) -> int:
    box = _with_retry(lambda: boxscore.BoxScore(game_id))
...
def _get_play_by_play_via_stats_raw(game_id: str, period: int | None) -> dict:
    ...
    """
    try:
        pbp = _with_retry(lambda: playbyplayv3.PlayByPlayV3(game_id=game_id))
...
def _get_play_by_play_raw(game_id: str, period: int | None = None) -> dict:
    ...
    try:
        ...
        pbp = _with_retry(lambda: playbyplay.PlayByPlay(game_id))
...
def _get_boxscore_via_stats_raw(game_id: str) -> dict:
    ...
    """
    try:
        box = _with_retry(lambda: boxscoretraditionalv3.BoxScoreTraditionalV3(game_id=game_id))
...
def _get_hustle_stats_raw(game_id: str) -> dict:
    ...
    """
    try:
        hustle = _with_retry(lambda: boxscorehustlev2.BoxScoreHustleV2(game_id=game_id))
...
def _get_matchups_raw(game_id: str, player_name: str) -> dict:
    ...
    """
    try:
        matchups = _with_retry(lambda: boxscorematchupsv3.BoxScoreMatchupsV3(game_id=game_id))
